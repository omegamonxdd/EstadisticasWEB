...
/**
 * Variables de Estado Globales para la Pestaña de Probabilidad
 * currentProbMode: 'classic' (n/N) o 'multi' (P1 * P2...)
 * currentMultiSubMode: 'prob' (decimales) o 'count' (fracciones n/N)
 */
let currentProbMode = 'classic';
let currentMultiSubMode = 'prob';

/**
 * Alterna entre el modo Clásico y el modo Multiplicativo.
 * Actualiza la visibilidad de los contenedores y los estilos de los botones.
 */
function toggleProbMode(mode) {
    currentProbMode = mode;
    const classicInput = document.getElementById('prob-input-classic');
    const multiInput = document.getElementById('prob-input-multi');
    ...
    // Ocultar resultados previos al cambiar de modo
    resBox.classList.add('hidden');
    errBox.classList.add('hidden');

    if (mode === 'classic') {
        // Mostrar interfaz para P(A) = n/N
        classicInput.classList.remove('hidden');
        multiInput.classList.add('hidden');
        btnClassic.classList.add('btn-primary');
        btnClassic.classList.remove('btn-light');
        btnMulti.classList.add('btn-light');
        btnMulti.classList.remove('btn-primary');
    } else {
        // Mostrar interfaz para Eventos Sucesivos
        classicInput.classList.add('hidden');
        multiInput.classList.remove('hidden');
        btnClassic.classList.add('btn-light');
        btnClassic.classList.remove('btn-primary');
        btnMulti.classList.add('btn-primary');
        btnMulti.classList.remove('btn-light');
        
        // Si no hay filas de eventos, inicializar con las 5 solicitadas
        const container = document.getElementById('prob-events-container');
        if (container.children.length === 0) {
            for(let i = 0; i < 5; i++) addProbEventRow();
        }
    }
}

/**
 * Dentro del modo Multiplicativo, alterna entre ingresar probabilidades directas (0.5)
 * o cantidades por evento (n/N).
 */
function toggleMultiSubMode(subMode) {
    currentMultiSubMode = subMode;
    const btnProb = document.getElementById('btn-sub-prob');
    const btnCount = document.getElementById('btn-sub-count');
    
    // Actualizar estilos visuales de los sub-botones
    if (subMode === 'prob') {
        btnProb.classList.add('btn-primary');
        btnProb.classList.remove('btn-light');
        btnCount.classList.add('btn-light');
        btnCount.classList.remove('btn-primary');
    } else {
        btnProb.classList.add('btn-light');
        btnProb.classList.remove('btn-primary');
        btnCount.classList.add('btn-primary');
        btnCount.classList.remove('btn-light');
    }
    
    // Limpiar y regenerar las filas para que coincidan con el nuevo tipo de input
    const container = document.getElementById('prob-events-container');
    const count = container.children.length;
    container.innerHTML = '';
    for (let i = 0; i < count; i++) addProbEventRow();
}

/**
 * Añade una nueva fila de evento al contenedor multiplicativo.
 * Límite máximo: 5 eventos.
 */
function addProbEventRow() {
    const container = document.getElementById('prob-events-container');
    if (container.children.length >= 5) return; // Restricción de negocio

    const id = `event-${Date.now()}-${container.children.length}`;
    const idx = container.children.length + 1;
    
    const div = document.createElement('div');
    div.id = id;
    div.className = 'fade-in';
    // Estilo en línea para asegurar el layout horizontal de los eventos sucesivos
    div.style.cssText = 'display: flex; align-items: center; gap: 1rem; background: white; padding: 0.75rem; border-radius: 1rem; border: 1px solid var(--border-color);';
    
    if (currentMultiSubMode === 'prob') {
        // Renderizar input para probabilidad decimal (Ej: 0.25)
        div.innerHTML = `
            <span style="font-weight: 900; color: var(--primary); min-width: 55px;">P(E${idx})</span>
            <input type="number" step="0.0001" min="0" max="1" class="input-number multi-p-input" style="margin: 0; font-size: 1.1rem; padding: 0.5rem;" placeholder="Ej: 0.5">
            <button onclick="removeProbEventRow('${id}')" style="color: var(--text-light); background: none; border: none; cursor: pointer; font-size: 1.25rem; font-weight: 900;">×</button>
        `;
    } else {
        // Renderizar inputs para n/N (Ej: 2 de 10)
        div.innerHTML = `
            <span style="font-weight: 900; color: var(--primary); min-width: 40px;">E${idx}</span>
            <input type="number" class="input-number multi-n-input" style="margin: 0; font-size: 0.9rem; padding: 0.5rem;" placeholder="n">
            <span style="font-weight: 900; color: var(--text-light);">/</span>
            <input type="number" class="input-number multi-N-input" style="margin: 0; font-size: 0.9rem; padding: 0.5rem;" placeholder="N">
            <button onclick="removeProbEventRow('${id}')" style="color: var(--text-light); background: none; border: none; cursor: pointer; font-size: 1.25rem; font-weight: 900;">×</button>
        `;
    }
...
/**
 * Elimina un evento específico y re-etiqueta los restantes para mantener el orden E1, E2...
 */
function removeProbEventRow(id) {
    const container = document.getElementById('prob-events-container');
    if (container.children.length <= 1) return; // Mantener al menos un input
    const element = document.getElementById(id);
    if (element) element.remove();
    
    // Re-etiquetar visualmente los eventos sucesivos tras una eliminación
    Array.from(container.children).forEach((row, idx) => {
        const span = row.querySelector('span');
        if (currentMultiSubMode === 'prob') {
            span.textContent = `P(E${idx + 1})`;
        } else {
            span.textContent = `E${idx + 1}`;
        }
    });
}

/**
 * Calcula la probabilidad clásica P(A) = n/N.
 */
function calculateProbability() {
    ...
    // Validación de entrada para el modelo clásico
    ...
    const prob = n / N;
    const common = gcd(Math.round(n), Math.round(N)); // Simplificación de fracción
...
/**
 * Motor de Cálculo para la Regla Multiplicativa de Probabilidades Independientes.
 * Aplica P_total = P1 * P2 * ... * Pn
 */
function calculateMultiProb() {
    const resBox = document.getElementById('prob-results-container');
    const errBox = document.getElementById('prob-error-box');
    resBox.classList.add('hidden');
    errBox.classList.add('hidden');

    let totalProb = 1;      // Neutro de la multiplicación
    let combinedNum = 1;    // Numerador para la fracción final
    let combinedDen = 1;    // Denominador para la fracción final
    let factorCount = 0;    // Contador para verificar cuántos campos llenó el usuario

    try {
        if (currentMultiSubMode === 'prob') {
            // Caso: Probabilidades decimales directas
            const inputs = document.querySelectorAll('.multi-p-input');
            inputs.forEach(input => {
                if (input.value.trim() !== '') {
                    const val = parseFloat(input.value);
                    if (isNaN(val) || val < 0 || val > 1) throw "Ingresa probabilidades válidas (0 a 1).";
                    totalProb *= val;
                    
                    // Convertir decimal a fracción para el resultado visual
            ...
            // Caso: Cantidades discretas n/N
            const rows = document.getElementById('prob-events-container').children;
            Array.from(rows).forEach(row => {
                const nInput = row.querySelector('.multi-n-input');
                const NInput = row.querySelector('.multi-N-input');
                
                // Procesar solo si ambos campos de la fila están llenos (Ajuste dinámico)
                if (nInput.value.trim() !== '' && NInput.value.trim() !== '') {
                    const n = parseFloat(nInput.value);
                    const N = parseFloat(NInput.value);
                    if (isNaN(n) || isNaN(N) || N <= 0 || n < 0) throw "Valores n/N inválidos.";
                    totalProb *= (n / N);
                    combinedNum *= n;
                    combinedDen *= N;
                    factorCount++;
        ...
        // Si el usuario no ingresó nada, abortar
        if (factorCount === 0) throw "Ingresa al menos una probabilidad.";

        // Simplificar la fracción resultante usando el Máximo Común Divisor (Euclides)
        const common = gcd(Math.round(combinedNum), Math.round(combinedDen));
        
        // Actualizar UI con resultados precisos
        document.getElementById('res-prob-decimal').textContent = totalProb.toFixed(6);
        document.getElementById('res-prob-fraction').textContent = `${Math.round(combinedNum/common)}/${Math.round(combinedDen/common)}`;
        document.getElementById('res-prob-percent').textContent = (totalProb * 100).toFixed(4) + '%';
        resBox.classList.remove('hidden');

    } catch (e) {
        // Manejo de errores dinámico
        document.getElementById('prob-error-text').textContent = e;
        errBox.classList.remove('hidden');
    }
}

/**
 * Utilidad para convertir un decimal a su fracción más simple.
 * Ejemplo: 0.25 -> { n: 1, d: 4 }
 */
function decimalToFraction(decimal) {
    if (decimal === 0) return { n: 0, d: 1 };
    let s = decimal.toString();
    if (s.indexOf('.') === -1) return { n: parseInt(s), d: 1 };
    
    let parts = s.split('.');
    let den = Math.pow(10, parts[1].length); // Base 10 según la cantidad de decimales
 ...
 * Fundamental para la simplificación de fracciones de probabilidad.
 */
function gcd(a, b) {
